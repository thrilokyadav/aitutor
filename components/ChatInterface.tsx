         ...
         <div className={`p-2 flex-1 overflow-hidden ${isPanelCollapsed ? 'flex flex-col items-center' : ''}`}>
            <div className={`flex items-center mb-4 ${isPanelCollapsed ? 'flex-col gap-4' : 'justify-between'}`}>
                 <button 
                    ...
                    className={`flex items-center justify-center gap-2 p-2 text-white rounded-md hover:opacity-90 transition-opacity w-full ${isPanelCollapsed ? '' : 'bg-gradient-to-r from-[rgb(var(--color-primary))] to-[rgb(var(--color-accent))] animated-gradient-bg'}`}
                    title="New Chat"
                >
                    <PlusIcon /> 
            ...
            {!isPanelCollapsed && (
                <div className="flex-1 overflow-y-auto">
                    {sessions.map(session => (
                        <div key={session.id} className={`p-2 rounded-md mb-2 cursor-pointer group flex justify-between items-center ${activeSessionId === session.id ? 'bg-[rgb(var(--color-card))] border border-[rgb(var(--color-primary))]' : 'hover:bg-[rgb(var(--color-input))]'}`} onClick={() => setActiveSessionId(session.id)}>
                        {editingSessionId === session.id ? (
                            <input 
                                ...
                                className="bg-[rgb(var(--color-input))] text-white p-1 rounded w-full"
                                autoFocus
                            />
                        ) : (
                            <>
        ...
        <div className="flex-1 overflow-y-auto pr-4">
          {!activeSession || activeSession.messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-[rgb(var(--color-text-secondary))]">
              ...
              <h2 className="text-2xl font-semibold">EASYWAY AI Tutor</h2>
              <p className="mt-2">Ask me anything about the ALL GOV syllabus!</p>
            </div>
          ) : (
        ...
        <div className="mt-6">
          ...
          <div className="flex items-center justify-end mt-2 gap-2 text-xs">
            ...
            <select
                ...
                className="bg-[rgb(var(--color-input))] border border-[rgb(var(--color-border))] rounded-md py-1 pl-2 pr-8 text-[rgb(var(--color-text-secondary))] text-xs focus:ring-1 focus:ring-[rgb(var(--color-primary))] focus:outline-none"
            >
