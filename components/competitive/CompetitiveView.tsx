...
const CompetitiveView: React.FC = () => {
  ...
  if (!user) {
    return (
      <div className="p-6">
        <h2 className="text-xl font-semibold mb-2">Competitive Quizzes</h2>
        <p className="text-[rgb(var(--color-text-secondary))] mb-3">Sign in to participate in competitive quizzes.</p>
        <button className="px-4 py-2 rounded-md bg-[rgb(var(--color-primary))] text-white" onClick={() => signInWithGoogle()}>Sign in with Google</button>
      </div>
    );
  }
    ...
    return (
      <div className="p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="text-sm bg-[rgb(var(--color-input))] px-3 py-1 rounded-md">Time left: {timeLeft ?? 0}s</div>
        </div>
        {resultMsg && <div className="p-3 rounded bg-[rgb(var(--color-input))]">{resultMsg}</div>}
        <div className="space-y-3">
          {payload.map((q, idx) => (
            <Card key={q.question_id}>
              <div className="text-sm text-[rgb(var(--color-text-secondary))] mb-1">Question {idx + 1}</div>
        ...
        <div className="flex items-center gap-2">
          <button disabled={submitting} onClick={handleSubmit} className="px-4 py-2 rounded-md bg-[rgb(var(--color-primary))] text-white disabled:bg-slate-600">Submit</button>
          <button onClick={() => { setActiveQuiz(null); setPayload(null); setAnswers({}); setTimeLeft(null); setResultMsg(null); }} className="px-4 py-2 rounded-md bg-[rgb(var(--color-input))]">Cancel</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <h2 className="text-xl font-semibold">Competitive Quizzes</h2>
      {/* Tabs and controls */}
      <div className="flex items-center gap-2">
      ...
      {loading && <div className="flex items-center gap-2"><LoadingSpinner /> Loading…</div>}
          ...
          <>
            <div className="grid md:grid-cols-2 gap-4">
              {pageItems.length === 0 && !loading ? (
                <div className="text-[rgb(var(--color-text-secondary))]">No quizzes.</div>
              ) : pageItems.map(q => (
          <Card key={q.id}>
            <div className="flex items-center justify-between">
              <div>
              ...
              {isLive(q) ? (
                <div className="flex items-center gap-2">
                  <button onClick={() => startQuiz(q)} className="px-3 py-1 rounded-md bg-[rgb(var(--color-primary))] text-white">Start</button>
                ...
                <span className="px-3 py-1 rounded-md bg-[rgb(var(--color-input))]">
                  {(() => {
                    ...
                    const mm = Math.floor(s / 60);
                    const ss = s % 60;
                    return `Starts in ${mm}m ${ss}s`;
                  })()}
                </span>
              ) : (
                <span className="px-3 py-1 rounded-md bg-[rgb(var(--color-input))]">Closed</span>
              )}
            </div>
            {q.description && <p className="mt-2 text-sm text-[rgb(var(--color-text-secondary))]">{q.description}</p>}
            {q.quiz_focus && <p className="mt-1 text-xs text-[rgb(var(--color-text-secondary))]"><span className="font-semibold">Focus:</span> {q.quiz_focus}</p>}
            {attempted[q.id] && (
              <p className="mt-2 text-xs text-amber-300">You have already taken this quiz. You can\'t retake it.</p>
            )}
            <div className="mt-2 flex items-center gap-2">
              <button className="px-3 py-1 rounded-md bg-[rgb(var(--color-input))]" onClick={() => viewLeaderboard(q)}>View Leaderboard</button>
              {q.published_leaderboard ? <span className="text-xs text-emerald-300">Published</span> : <span className="text-xs text-[rgb(var(--color-text-secondary))]">Unpublished</span>}
            </div>
          </Card>
        ))}
            </div>
            ...
            <div className="flex items-center justify-between mt-3">
              <div className="text-xs text-[rgb(var(--color-text-secondary))]">Page {currentPage} of {totalPages}</div>
              <div className="flex items-center gap-2">
                <button disabled={currentPage <= 1} onClick={() => setPage(p => Math.max(1, p - 1))} className="px-2 py-1 rounded bg-[rgb(var(--color-input))] disabled:opacity-50">Prev</button>
                <button disabled={currentPage >= totalPages} onClick={() => setPage(p => Math.min(totalPages, p + 1))} className="px-2 py-1 rounded bg-[rgb(var(--color-input))] disabled:opacity-50">Next</button>
              </div>
            </div>
          </>
        );
      ...
      <div className="mt-3">
        <button onClick={fetchQuizzes} className="px-3 py-1 rounded-md bg-[rgb(var(--color-input))]">Refresh</button>
      </div>
      ...
      {boardQuiz && (
        <Card>
          <div className="flex items-center justify-between">
            <h3 className="font-semibold">Leaderboard — {boardQuiz.title}</h3>
            <button className="px-2 py-1 text-xs rounded bg-[rgb(var(--color-input))]" onClick={() => { setBoardQuiz(null); setLeaderboard(null); }}>Close</button>
          </div>
          {loadingBoard ? (
            <div className="flex items-center gap-2 mt-2"><LoadingSpinner /> Loading…</div>
            ...
            <p className="text-sm text-[rgb(var(--color-text-secondary))] mt-2">No leaderboard yet or not published.</p>
          )}
        </Card>
      )}
    </div>
